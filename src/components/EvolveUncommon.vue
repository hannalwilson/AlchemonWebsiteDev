...
export default {
  ...
  setup () {
    for (const item in alchemon) {
      if (alchemon[item].rarity === 4) {
        uncommons[item] = alchemon[item]
      }
    }
  },
  methods: {
    setAlchemon (name) {
      for (const item in uncommons) {
        if (uncommons[item].name === name) {
          console.log(uncommons[item])
          const id = uncommons[item].appID
          const evolved = uncommons[item].id
          const traded = uncommons[item].tradedCard
          const token = uncommons[item].alchecoinAssetID
          const cost = uncommons[item].requiredAmountOfAlchecoin
          const tradedAddress = uncommons[item].tradeInStoreAddress
          const address = localStorage.userAddress
          const wallet = localStorage.userWallet
          this.evolveAlchemon(id, evolved, traded, tradedAddress, address, wallet, token, cost)
          break
        }
      }
    },
    async evolveAlchemon (appID, evolvedAlchemon, tradedAlchemon, tradeInAddress, address, wallet, paymentID, cost) {
      if (!wallet) {
      ...
      const quickEvolveOneResponse = await axios.post(`${apiURL}/quickEvolveAlchOne`, {
        ...
        tradeInStoreAddress: tradeInAddress,
